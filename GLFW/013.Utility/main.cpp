...
public:
    ...
    virtual void update(double timeval) override {
        ...
        GLuint v_pos = glGetUniformLocation(renderingProgram,"v_matrix");
        ...
        glUniformMatrix4fv(v_pos, 1, GL_FALSE, glm::value_ptr(v_mat));
        
        float timeFactor = (float)timeval;
        GLuint tif = glGetUniformLocation(renderingProgram, "tf");
        
        glUniform1f(tif, timeFactor);
        glBindBuffer(GL_ARRAY_BUFFER, vbo[0]);
